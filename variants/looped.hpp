...
public:
  blocking_pseudo_execution_policy(size_t dim) : dim(dim) {
    this->add_blocking(std::vector<T>(
        dim, static_cast<T>(1))); // , std::vector<T>(dim, hpx::parallel::seq)
  }

  blocking_pseudo_execution_policy &add_blocking(const std::vector<T> &block) {
    //            std::vector<const hpx::parallel::parallel_execution_policy>
    //            &dim_execution
    //            ) {
    if (block.size() != dim) { //  || dim_execution.size() != dim
      throw;
    }
    blocking_configuration.push_back(
        block); // std::make_pair(block, dim_execution)
    return *this;
  }

  std::vector<T> pop() { // std::pair<std::vector<T>>, std::vector<const
...
void iterate_indices(blocking_pseudo_execution_policy<T> policy,
  ...
  size_t dim = min.size();
  //    std::pair<std::vector<T>, const
  //    hpx::parallel::parallel_execution_policy> cur_policy =
  ...
  if (policy.is_last_blocking_step()) {
    ...
    hpx::parallel::for_each_n(
        ...
        [dim, policy, &block, f](const std::vector<size_t> &cur_index) {
          ...
          iterate_indices(policy, recursive_min, recursive_max, f);
        });
  }
}
//
...
public:
  ...
  std::vector<double> matrix_multiply() {
    ...
    //        hpx::parallel::par;
    //        const hpx::parallel::parallel_execution_policy &t =
    //        hpx::parallel::seq;
