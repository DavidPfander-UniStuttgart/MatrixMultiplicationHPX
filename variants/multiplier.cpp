...
HPX_REGISTER_COMPONENT(hpx::components::component<multiply_components::multiplier>,
		       multiplier);

HPX_REGISTER_ACTION(multiply_components::multiplier::calculate_submatrix_action);
...
std::vector<double> multiplier::calculate_submatrix(std::uint64_t x,
  ...
  if (!transposed) {
    ...
    if (block_input == 0 || block_result < 4) {
      kernel::kernel_transposed(A, B, C, N, x, y,
						block_result);
    } else {
      kernel::kernel_transposed_blocked(A, B, C, N, x, y,
