...
public:
  index_iterator() : stride(0), x(0), y(0) {}
...
public:
  dim_index_iterator() : dim(0), cur_index(0) {}
...
public:
  ...
  std::vector<double> matrix_multiply() {
    ...
    //                    hpx::parallel::for_loop(hpx::parallel::par, 0,
    //                    result_blocks, [this, block_x, &C](size_t block_y) {
    hpx::parallel::for_each_n(
        hpx::parallel::par, dim_index_iterator<size_t>(2, result_blocks, 1),
        result_blocks * result_blocks,
        [this, &C](const std::vector<size_t> &cur_index) {

          size_t block_x = cur_index[0];
          size_t block_y = cur_index[1];
          //                    hpx::cout << "block_x: " << block_x << "
          //                    block_y: " << block_y << std::endl <<
          //                    hpx::flush;
          // should the matrix be smaller than 128 (all bigger than 128 are also
          // multiples of 128)
          ...
          // block the bands in the (big) input matrices, in this loop algorithm
          // is within small block matrix
          for (size_t k_block = 0; k_block < N; k_block += block_input) {
            // should the matrix be smaller than 128 (all bigger than 128 are
            // also multiples of 128)
            //                        size_t k_input_end = std::min(k_block +
            //                        block_input, N);

            // move input matrix into cache (and suppress conflict misses)
            for (size_t x = 0; x < block_result; x++) {
              for (size_t k = 0; k < block_input; k++) {
                A_small[x * block_input + k] =
                    A[(outer_x + x) * N + k_block + k];
              }
            }
            ...
            for (size_t y = 0; y < block_result; y++) {
              for (size_t k = 0; k < block_input; k++) {
                B_small[y * block_input + k] =
                    B[(outer_y + y) * N + k_block + k];
              }
            }

            // process all (small) block result matrix components sequentially
            // (to benefit from the cache within a single core)
            hpx::parallel::for_each_n(
                hpx::parallel::seq,
                index_iterator<double>(C_inner, block_result),
                block_result * block_result,
                [this, &A_small, &B_small](
                    std::tuple<size_t, size_t, std::reference_wrapper<double>>
                        t) {
                  ...
                  auto A_submatrix_end =
                      A_small.begin() + (x + 1) * block_input;
                  ...
                  value +=
                      std::inner_product(A_submatrix_begin, A_submatrix_end,
                                         B_submatrix_begin, 0.0);
                });
          }

          // write (small) block result matrix back into (big) overall result
          // matrix
          for (size_t x = 0; x < block_result; x++) {
            for (size_t y = 0; y < block_result; y++) {
              C[(outer_x + x) * N + (outer_y + y)] =
