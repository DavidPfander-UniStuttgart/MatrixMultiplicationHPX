...
void matrix_multiply_static::insert_submatrix(
...
std::vector<matrix_multiply_work> matrix_multiply_static::create_work_packages(
	...
	while (work.size() < num_localities) {
		...
		for (auto offset : offsets) {
			work.emplace_back(w.x + std::get<0>(offset),
					w.y + std::get<1>(offset), n_new);
		}

		std::cout << "-----------------------" << std::endl;
		for (auto &w : work) {
			std::cout << "x: " << w.x << " y: " << w.y << " n: " << w.N
					<< std::endl;
		}
...
std::vector<double> matrix_multiply_static::matrix_multiply() {
	hpx::cout << "using static distributed algorithm" << std::endl
			<< hpx::flush;
	size_t num_localities = hpx::get_num_localities().get();
	double exponent = std::log2(num_localities);
	if (exponent != std::trunc(exponent)) {
		hpx::cout << "warning: number of localities is not a power of 2!"
				<< std::endl << hpx::flush;
	}
	if (num_localities % 4 != 0) {
		hpx::cout << "warning: number of localities is not divisible by 4!"
				<< std::endl << hpx::flush;
	}
	...
	std::vector<hpx::components::client<matrix_multiply_multiplier>> multipliers =
			hpx::new_<hpx::components::client<matrix_multiply_multiplier>[]>(
					policy, num_localities, N, A, B, transposed, block_input,
					verbose).get();
	...
	std::vector<hpx::components::client<matrix_multiply_recursive>> recursives =
			hpx::new_<hpx::components::client<matrix_multiply_recursive>[]>(
					policy, num_localities, small_block_size, verbose).get();
	...
	std::vector<matrix_multiply_work> work = this->create_work_packages(
			num_localities);
	...
	while (work_iterator != work.end()) {
		if (verbose >= 1) {
			hpx::cout << "warning: extra work for first locality!" << std::endl
					<< hpx::flush;
		}
