...
void pvn_compare(const std::string &scenario_name,
  ...
  evaluate_pvn(parameters, pvn_csv_file, "L1_K_STEP", "16");
  ...
  evaluate_pvn(parameters, pvn_csv_file, "L2_K_STEP",
               parameters.get_by_name("L1_K_STEP")->get_value());
...
void do_tuning(tuner_t &tuner, parameter_set_type &ps,
  ...
  for (size_t restart = 0; restart < detail::restarts; restart++) {
    ...
    if (detail::use_pvn) {
      ...
      ps.get_by_name("L1_K_STEP")->set_value_unsafe("16");
      ps.get_by_name("L2_X")->set_value_unsafe("20");
      ps.get_by_name("L2_Y")->set_value_unsafe("16");
      ps.get_by_name("L2_K_STEP")->set_value_unsafe("32");
      ps.get_by_name("KERNEL_OMP_THREADS")
          ->set_value_unsafe(std::to_string(detail::thread_values[0]));
    }
    // autotune::parameter_value_set pv = autotune::to_parameter_values(ps);
    // autotune::parameter_values_to_file(pv,
    //                                    scenario_name + std::string(".json"));
    // autotune::parameter_value_set pv2 = autotune::parameter_values_from_file(
    //     scenario_name + std::string(".json"));
    // for (auto &p : pv2) {
    //   std::cout << p.first << " = " << p.second << std::endl;
    // }

    // ps.get_by_name("KERNEL_NUMA")->set_value_unsafe("1");
    // ps.get_by_name("KERNEL_SCHEDULE")->set_value_unsafe("1");
    // ps.get_by_name("X_REG")->set_value_unsafe("4");
    // ps.get_by_name("Y_BASE_WIDTH")->set_value_unsafe("2");
    // ps.get_by_name("L1_X")->set_value_unsafe("36");
    // ps.get_by_name("L1_Y")->set_value_unsafe("16");
    // ps.get_by_name("L1_K_STEP")->set_value_unsafe("64");
    // ps.get_by_name("L2_X")->set_value_unsafe("72");
    // ps.get_by_name("L2_Y")->set_value_unsafe("128");
    // ps.get_by_name("L2_K_STEP")->set_value_unsafe("64");
    // ps.get_by_name("KERNEL_OMP_THREADS")->set_value_unsafe("64");
...
int main(int argc, char **argv) {
  ...
  autotune::countable_continuous_parameter p2("Y_BASE_WIDTH", 1, 1, 1, 5); // 5
  autotune::countable_continuous_parameter p3("L1_X", 10, 5, 10, 40);      // 8
  autotune::countable_continuous_parameter p4("L1_Y", 8, 8, 8, 64);        // 8
  autotune::countable_continuous_parameter p5("L1_K_STEP", 16, 16, 16,
                                              128);                     // 8
  autotune::countable_continuous_parameter p6("L2_X", 20, 10, 20, 100); // 8
  autotune::countable_continuous_parameter p7("L2_Y", 16, 16, 16, 128); // 8
  autotune::countable_continuous_parameter p8("L2_K_STEP", 32, 32, 32,
                                              256); // 7
  ...
  autotune::fixed_set_parameter<size_t> p9("KERNEL_OMP_THREADS",
