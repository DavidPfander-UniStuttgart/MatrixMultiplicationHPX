...
void do_tuning_countable(tuner_t &tuner, const std::string &scenario_name) {
  ...
  for (size_t restart = 0; restart < detail::restarts; restart++) {
    ...
    while (!valid_start_found) {
      for (size_t parameter_index = 0;
           parameter_index < detail::parameters.size(); parameter_index++) {
        auto &p = detail::parameters[parameter_index];
    ...
    tuner.set_write_measurement(scenario_name + std::to_string(restart));
    ...
    std::vector<double> C =
        autotune::combined_kernel(detail::N, detail::A, detail::B,
                                  detail::repetitions, detail::duration_kernel);
    ...
    double flops = 2 * static_cast<double>(detail::N) * //TODO: need padded dimensions
  ...
  for (size_t parameter_index = 0; parameter_index < detail::parameters.size();
       parameter_index++) {
    auto &p = detail::parameters[parameter_index];
    p->set_initial();
  }
...
//     defined(DO_GREEDY_NEIGHBOR_SEARCH_SPLIT)
  // autotune::countable_set parameters;
  detail::parameters.add_parameter(p0a);
  detail::parameters.add_parameter(p0b);
  detail::parameters.add_parameter(p1);
  detail::parameters.add_parameter(p2);
  detail::parameters.add_parameter(p3);
  detail::parameters.add_parameter(p4);
  detail::parameters.add_parameter(p5);
  detail::parameters.add_parameter(p6);
  detail::parameters.add_parameter(p7);
  detail::parameters.add_parameter(p8);
  detail::parameters.add_parameter(p9);
  // #endif

  // #if defined(DO_MONTE_CARLO)
  autotune::randomizable_set randomizable_parameters;
  randomizable_parameters.add_parameter(p0a);
  randomizable_parameters.add_parameter(p0b);
...
///////////// end new adjust
    ...
    for (size_t restart = 0; restart < restarts; restart++) {
      ...
      autotune::countable_set optimal_parameters;
      {
        ...
        optimal_parameters = tuner.tune(m.N_org, m.A_org, m.B_org,
                                        m.repetitions, detail::duration_kernel);
        std::chrono::high_resolution_clock::time_point end =
            std::chrono::high_resolution_clock::now();
        double tuning_duration =
            std::chrono::duration<double>(end - start).count();
      ...
      double gflop = flops / 1E9;
      std::cout << "optimal inner_duration (line search): "
                << detail::duration_kernel << std::endl;
      std::cout << "[N = " << N << "] performance: "
                << ((repetitions * gflop) / detail::duration_kernel) << "GFLOPS"
                << std::endl;
    }
    for (size_t parameter_index = 0; parameter_index < parameters.size();
         parameter_index++) {
  ...
  // tune with parallel line search, large scenario
  {
    std::cout << "----------------- starting tuning with parallel line search, "
                 "large ------------ "
              << std::endl;
    // std::uint64_t N = 4096; // TODO: move global
    size_t line_search_steps = 50;
    autotune::tuners::parallel_line_search tuner(
        autotune::combined_kernel, detail::parameters, line_search_steps);
    tuner.set_parameter_adjustment_functor(parameter_adjustment_functor);
    do_tuning_countable(tuner, scenario_name + "parallel_line_tune");
  }
// {
//   std::cout << "----------------- starting tuning with parallel line search,
//   "
...
//       double tuning_duration =
//           std::chrono::duration<double>(end - start).count();
//       tuner_duration_file << "parallel_line_search_large, " <<
//       tuning_duration
//                           << std::endl;
//     }
...
//       std::chrono::high_resolution_clock::time_point end =
//           std::chrono::high_resolution_clock::now();
//       double tuning_duration = std::chrono::duration<double>(end -
//       start).count();
//       tuner_duration_file << "parallel_line_search_noisy, " <<
...
//     duration_kernel
//               << std::endl;
//     std::cout << "[N = " << N << "] performance: " << ((repetitions * gflop)
//     / duration_kernel)
//               << "GFLOPS" << std::endl;
//   }
//   for (size_t parameter_index = 0; parameter_index < parameters.size();
