...
void matrix_multiply_static_improved::insert_submatrix(const std::vector<double> &submatrix, const matrix_multiply_work &w) {
...
bool matrix_multiply_static_improved::fulfills_constraints(std::vector<uint64_t> &total_work) {
  auto result = std::minmax(std::begin(total_work), std::end(total_work));
  uint64_t min_work = *result.first;
  uint64_t max_work = *result.second;
  if (max_work - min_work < max_work_difference) {
    return true;
  }
  if (static_cast<double>(max_work - min_work) / static_cast<double>(min_work) < max_relative_work_difference) {
    return true;
  }
  return false;
}

std::vector<std::vector<matrix_multiply_work>> matrix_multiply_static_improved::create_schedule(size_t num_localities) {
  ...
  while (!this->fulfills_constraints(total_work)) {
    ...
    for (size_t j = 0; j < all_work[max_index].size(); j++) {
      ...
      uint64_t work_difference = (max_work - split_size) - (min_work + split_size);
    ...
    all_work.erase(all_work.begin() + work_to_balance_index);

    size_t n_new = w.N / 2;
    std::vector<std::tuple<size_t, size_t>> offsets =
      {{ 0, 0 }, {0 + n_new, 0 }, { 0, 0 + n_new }, { 0 + n_new, 0 + n_new } };

    all_work[max_index].emplace_back(w.x + std::get<0>(offsets[0]), w.y + std::get<1>(offsets[0]), n_new);
    all_work[max_index].emplace_back(w.x + std::get<0>(offsets[1]), w.y + std::get<1>(offsets[1]), n_new);
    
    all_work[min_index].emplace_back(w.x + std::get<0>(offsets[2]), w.y + std::get<1>(offsets[2]), n_new);
    all_work[min_index].emplace_back(w.x + std::get<0>(offsets[3]), w.y + std::get<1>(offsets[3]), n_new);
  }
  return all_work;
}

std::vector<double> matrix_multiply_static_improved::matrix_multiply() {
  hpx::cout << "using pseudodynamic distributed algorithm" << std::endl << hpx::flush;
  ...
  std::vector<std::vector<matrix_multiply_work>> all_work = this->create_schedule(num_localities);
