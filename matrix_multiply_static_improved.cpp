    ...
    while (!this->fulfills_constraints(total_work)) {
        ...
        for (size_t max_index : max_indices) {
            ...
            for (size_t j = 0; j < all_work[max_index].size(); j++) {
                ...
                int64_t work_difference = std::abs(
                        static_cast<int64_t>(max_work - split_size)
        ...
        if (!found) {
            if (verbose >= 1) {
                std::cout
                        << "aborted due to no valid work package to distribute found"
                        << std::endl;
            }
            break;
        }
        ...
        all_work[valid_max_index].erase(
                all_work[valid_max_index].begin()
