...
void kernel_transposed_blocked(std::vector<T> &A,
    ...
    std::vector<T, boost::alignment::aligned_allocator<T, 32> > A_small(block_input * block_result);
    std::vector<T, boost::alignment::aligned_allocator<T, 32> > B_small(block_input * block_result);

//    T *A_small = static_cast<T *>(__builtin_assume_aligned(A_small_aligned.data(), 32));
//    T *B_small = static_cast<T *>(__builtin_assume_aligned(B_small_aligned.data(), 32));
