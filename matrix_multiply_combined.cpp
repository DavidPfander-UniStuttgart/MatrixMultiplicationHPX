...
namespace combined {
  ...
  void matrix_multiply_combined::verify_blocking_setup() {
    ...
    if (!((L2_Y % L1_Y == 0) && (L3_Y % L2_Y == 0))) {
      std::cout << "error: y direction blocking not set up correctly" << std::endl;
      throw;
    }
    if (!((L2_K_STEP % L1_K_STEP == 0) && (L3_K_STEP % L2_K_STEP == 0))) {
      std::cout << "error: k direction blocking not set up correctly" << std::endl;
      throw;
    }
  }
  ...
  matrix_multiply_combined::matrix_multiply_combined(size_t N, std::vector<double> &A_org,
    ...
    N_org(N), A(A_org), B(B_org), transposed(transposed), block_result(block_result), block_input(block_input), repetitions(repetitions), verbose(verbose) {
  ...
  std::vector<double> matrix_multiply_combined::matrix_multiply(double &duration) {
    ...
    std::vector<double, boost::alignment::aligned_allocator<double, 32>> A_trans(K_size * X_size);
    for (size_t l1_x = 0; l1_x < X_size/L1_X; l1_x += 1) {
      for (size_t l1_k = 0; l1_k < K_size/L1_K_STEP; l1_k += 1) {
	size_t base_index = (L1_X * L1_K_STEP) * (l1_k * (X_size/L1_X) + l1_x); // look up submatrix
	for (size_t x = 0; x < L1_X; x++) {
	  for (size_t k = 0; k < L1_K_STEP; k++) {
	    A_trans[base_index + k * L1_X + x] = A[(l1_x * L1_X + x) * K_size + (l1_k * L1_K_STEP + k)];
	  }
	}
      }
    }
    ...
    std::vector<double, boost::alignment::aligned_allocator<double, 32>> B_padded(K_size * Y_size);
    for (size_t l1_y = 0; l1_y < (Y_size/L1_Y); l1_y += 1) {
      for (size_t l1_k = 0; l1_k < (K_size/L1_K_STEP); l1_k += 1) {
	size_t base_index = (L1_Y * L1_K_STEP) * (l1_k * (Y_size/L1_Y) + l1_y); // look up submatrix
	for (size_t y = 0; y < L1_Y; y++) {
	  for (size_t k = 0; k < L1_K_STEP; k++) {
	    B_padded[base_index + k * L1_Y + y] = B[(l1_k * L1_K_STEP + k) * Y_size + (l1_y * L1_Y + y)];
	  }
	}
      }
    }
    ...
    for (size_t rep = 0; rep < repetitions; rep++) {
      ...
      policy.add_blocking({L3_X, L3_Y, L3_K_STEP}, {true, true, false}); // LLC blocking

      std::chrono::high_resolution_clock::time_point start = std::chrono::high_resolution_clock::now();
      
      iterate_indices<3>(policy, min, max,
			 [&C_padded, &A_trans, &B_padded, this](size_t l1_x, size_t l1_y, size_t l1_k) {
    ...
    for (size_t l1_x = 0; l1_x < X_size/L1_X; l1_x += 1) {
      for (size_t l1_y = 0; l1_y < Y_size/L1_Y; l1_y += 1) {
	size_t base_index = (L1_X * L1_Y) * (l1_x * (Y_size/L1_Y) + l1_y); // look up submatrix
	for (size_t x = 0; x < L1_X; x++) {
	  for (size_t y = 0; y < L1_Y; y++) {
	    // skip padding
	    if (l1_x * L1_X + x < N_org && l1_y * L1_Y + y < N_org) {
	      C_return.at((l1_x * L1_X + x) * N_org + (l1_y * L1_Y + y)) = C_padded.at(base_index + x * L1_Y + y);
	    }
	  }
	}
      }
    ...
    std::cout << "[X_size = " << X_size << ", Y_size = " << Y_size << ", K_size = " << K_size << "] inner performance: "
