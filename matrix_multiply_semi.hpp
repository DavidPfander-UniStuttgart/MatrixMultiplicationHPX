...
private:
    ...
    uint64_t verbose;
public:
    matrix_multiply_semi(size_t N, std::vector<double> &A,
...
//        {   hpx::parallel::par, hpx::parallel::par, hpx::parallel::seq};
        ...
        policy.add_blocking({block_result, block_result, block_input}, {true, true, false}); // L1 blocking
...
//                    C_conflict[x * N_fixed + y] += std::inner_product(A_submatrix_begin, A_submatrix_end, B_submatrix_begin, 0.0);
                    ...
                    for (size_t k_inner = 0; k_inner < 64; k_inner++) {
